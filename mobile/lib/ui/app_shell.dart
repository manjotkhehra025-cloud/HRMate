...
/// Authenticated shell — the webapp's chrome (Phase 8):
...
///
/// SAFETY (ARCHITECTURE.md §7): no negative padding, no const on
/// non-const-constructible widgets, no Container(color:)+decoration.
class AppShell extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    // Tab switches collapse the pushed pages first (profile, sheets closed)
    // so the shell never stacks on itself.
    void go(String path) {
    ...
    return Scaffold(
      body: SafeArea(
        ...
        child: Column(children: [
          _Header(onProfile: () => Navigator.of(context).push(
...
/// White top bar, exactly like the webapp header.
...
/// The raised emerald Punch circle (60 px, white ring, emerald glow).
///
/// The raise is a negative-Offset `Positioned` inside an unclipped Stack —
/// never negative `Padding` (that is a `padding.isNonNegative` assertion).
/// Layout slot stays 42 px (60 − 18).
...
/// Simple profile screen (full profile lands with the More phase).
          ...
          Row(children: [
            ...
            Expanded(
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Text(user?.name ?? '', style: const TextStyle(fontSize: 17, fontWeight: FontWeight.w800, color: HrBrand.heading)),
                const SizedBox(height: 2),
                Text(user?.code ?? '', style: const TextStyle(fontSize: 12.5, color: HrBrand.subInk)),
                const SizedBox(height: 6),
                Container(
                  padding: const EdgeInsets.symmetric(horizontal: 9, vertical: 3),
                  decoration: BoxDecoration(color: HrBrand.blueContainer, borderRadius: BorderRadius.circular(999)),
                  child: Text(user?.roleLabel ?? '',
                      style: const TextStyle(fontSize: 11, fontWeight: FontWeight.w700, color: HrBrand.blueDeep)),
                ),
              ]),
            ),
          ]),
