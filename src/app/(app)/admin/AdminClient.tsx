...
export default function AdminClient({
  ...
  const { t } = usePrefs();
  const tabs = [
    { key: "users", label: t("systemUsersAdmins"), icon: <Shield className="h-4 w-4" />, show: canUsers },
    { key: "employees", label: t("employeesStaff"), icon: <Users className="h-4 w-4" />, show: canUsers },
    { key: "permissions", label: t("rolePermissions"), icon: <UserCog className="h-4 w-4" />, show: canPermissions },
  ].filter((t) => t.show);
    ...
    <div className="space-y-6">
      ...
      <div className="flex gap-1 overflow-x-auto rounded-[14px] bg-[#EEF2F7] p-1" style={{ touchAction: "pan-x" }}>
        {tabs.map((tabItem) => (
          ...
          >
            <span className={tab === tabItem.key ? "text-[#1E6FE0]" : "text-[#8A97A8]"}>{tabItem.icon}</span>
            {tabItem.label}
          </button>
        ))}
      </div>
