...
export default function CalendarClient({ canManage = false, currentUserId }: CalendarClientProps) {
  ...
  const [listScope, setListScope] = useState<"month" | "year">("month");
  ...
  const loadCalendarData = async () => {
    try {
      setLoading(true);
      const monthQuery = viewMode === "list" && listScope === "year" ? "all" : String(currentMonth);
      const res = await fetch(`/api/calendar?year=${currentYear}&month=${monthQuery}`);
  ...
  useEffect(() => {
    loadCalendarData();
  }, [currentYear, currentMonth, viewMode, listScope]);
  ...
  // Calendar Grid Days Generation (Strictly uses filteredEvents)
  ...
  const calendarGrid = useMemo(() => {
    ...
    return grid;
  }, [currentYear, currentMonth, daysInMonth, filteredEvents]);
  ...
  const originUrl = typeof window !== "undefined" ? window.location.origin : "https://gdfoods.duckdns.org";
  const webcalUrl = originUrl.replace(/^https?:\/\//, "webcal://") + "/api/holidays/ical";
  const googleCalSubscribeUrl = `https://calendar.google.com/calendar/r?cid=${encodeURIComponent(originUrl + "/api/holidays/ical")}`;

  return (
    <div className="space-y-6 pb-12 animate-fade-in">
      {/* 1. HERO BIRTHDAY CELEBRATION BANNER */}
      ...
      <div className="flex flex-col gap-4 rounded-3xl border border-[#E2E8F0] bg-white p-4 sm:p-5 shadow-sm dark:border-[#1E293B] dark:bg-[#0F172A]">
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h1 className="text-[20px] sm:text-[24px] font-black text-[#0F172A] dark:text-white leading-tight">
                {viewMode === "list" && listScope === "year" ? `Full Year ${currentYear}` : `${currentMonthName} ${currentYear}`}
              </h1>
              <p className="text-[12px] font-medium text-[#64748B] dark:text-[#94A3B8]">
                GD Foods Mfg. (I) Pvt. Ltd. (Khadur Sahib Unit) · Official 11 Paid Holidays
              </p>
            </div>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <button
              ...
              className="inline-flex items-center gap-1.5 rounded-2xl bg-emerald-50 border border-emerald-200 px-3 py-2 text-[12px] font-bold text-emerald-700 hover:bg-emerald-100 active:scale-95 transition dark:bg-emerald-950/40 dark:text-emerald-300 dark:border-emerald-800"
              title="Sync factory holidays with Google / Phone Calendar"
            >
        ...
        {/* Filter Pills, List Scope & Search */}
        <div className="flex flex-col gap-3 pt-3 border-t border-[#F1F5F9] sm:flex-row sm:items-center sm:justify-between dark:border-[#1E293B]">
          ...
          <div className="flex items-center gap-2">
            {viewMode === "list" && (
              <div className="flex items-center rounded-xl border border-[#CBD5E1] bg-[#F8FAFC] p-0.5 text-[11px] font-bold dark:border-[#334155] dark:bg-[#0B132B]">
                <button
                  type="button"
                ...
                >
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-0.5">
                      {hasOff && <span className="h-2 w-2 rounded-full bg-rose-500 shadow-sm" title="Factory Off" />}
                      {hasFestival && <span className="h-2 w-2 rounded-full bg-emerald-500 shadow-sm" title="Observance" />}
                      {hasBday && <span className="h-2 w-2 rounded-full bg-pink-500 shadow-sm" title="Birthday" />}
                    </div>
                  </div>
          ...
          {selectedDateStr && (
            <div className="mt-5 rounded-2xl border border-[#CBD5E1] bg-[#F8FAFC] p-4 shadow-sm animate-fade-in dark:border-[#334155] dark:bg-[#0B132B]">
              ...
              {(!selectedDayEvents || selectedDayEvents.length === 0) ? (
                <p className="mt-3 text-[13px] text-[#64748B]">No special events scheduled (Regular Factory Working Day).</p>
              ) : (
      ...
      {/* 5. SYNC TO PHONE MODAL */}
      {syncModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-fade-in">
          <div className="w-full max-w-lg rounded-3xl border border-[#E2E8F0] bg-white p-6 shadow-2xl dark:border-[#1E293B] dark:bg-[#0F172A]">
            <div className="flex items-center justify-between border-b border-[#F1F5F9] pb-3 dark:border-[#1E293B]">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-[17px] font-black text-[#0F172A] dark:text-white">
                  Sync Factory Holidays with Phone
                </h3>
              </div>
              <button
                type="button"
            ...
            <p className="mt-3 text-[13px] text-[#64748B] dark:text-[#94A3B8]">
              Choose how you want to add the official 11 GD Foods factory holidays into your phone:
            </p>

            <div className="mt-4 space-y-3">
              {/* Option 1: 1-Click Google Calendar */}
              <a
                href={googleCalSubscribeUrl}
                target="_blank"
                rel="noopener noreferrer"
                onClick={() => setSyncModalOpen(false)}
                className="flex items-center justify-between gap-3 rounded-2xl border border-blue-200 bg-blue-50/70 p-3.5 transition hover:bg-blue-100 active:scale-98 dark:border-blue-900/50 dark:bg-blue-950/40"
              >
                <div className="flex items-center gap-3">
                  <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-[#1E6FE0] text-white">
                    <Globe className="h-5 w-5" />
                  </div>
                  <div>
                    <h4 className="text-[14px] font-bold text-[#0F172A] dark:text-white">
                      Google Calendar (1-Click Web Sync)
                    </h4>
                    <p className="text-[11.5px] text-[#64748B] dark:text-[#94A3B8]">
                      Opens Google Calendar directly to subscribe without downloading.
                    </p>
                  </div>
                </div>
                <ExternalLink className="h-4 w-4 text-[#1E6FE0]" />
              </a>

              {/* Option 2: Apple / Outlook / WebCal */}
              <a
                href={webcalUrl}
                onClick={() => setSyncModalOpen(false)}
                className="flex items-center justify-between gap-3 rounded-2xl border border-purple-200 bg-purple-50/70 p-3.5 transition hover:bg-purple-100 active:scale-98 dark:border-purple-900/50 dark:bg-purple-950/40"
              >
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h4 className="text-[14px] font-bold text-[#0F172A] dark:text-white">
                      iPhone / Device Calendar (WebCal)
                    </h4>
                    <p className="text-[11.5px] text-[#64748B] dark:text-[#94A3B8]">
                      Auto-syncs live updates directly to iPhone or Samsung calendar.
                    </p>
                  </div>
                </div>
              ...
              <a
                ...
                download="hrmate-holidays-2026.ics"
                onClick={() => setSyncModalOpen(false)}
                className="flex items-center justify-between gap-3 rounded-2xl border border-emerald-200 bg-emerald-50/70 p-3.5 transition hover:bg-emerald-100 active:scale-98 dark:border-emerald-900/50 dark:bg-emerald-950/40"
              >
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h4 className="text-[14px] font-bold text-[#0F172A] dark:text-white">
                      Download .ICS File (Offline Import)
                    </h4>
                    <p className="text-[11.5px] text-[#64748B] dark:text-[#94A3B8]">
                      Save file and tap "Import All" in MIUI, Samsung, or Google Calendar.
                    </p>
                  </div>
                </div>
            ...
            <div className="mt-4 rounded-2xl bg-[#F8FAFC] p-3 text-[11.5px] text-[#64748B] dark:bg-[#0B132B] dark:text-[#94A3B8] flex items-start gap-2">
              <Info className="h-4 w-4 shrink-0 text-[#1E6FE0] mt-0.5" />
              <span>
                <strong>Tip for Android/MIUI:</strong> When the downloaded file opens on your phone, choose <strong>MIUI Calendar</strong> or <strong>Google Calendar</strong> and tap <strong>"Add All / Import"</strong>.
              </span>
            </div>
          </div>
        </div>
      ...
      {/* 6. SUPER ADMIN ADD / EDIT HOLIDAY MODAL */}
      {modalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-fade-in">
