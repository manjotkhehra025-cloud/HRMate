...
export const POST = handle(async (req: NextRequest, ctx: { params: { id: string } }) => {
  ...
  if (!reason) {
    return fail(400, 'VALIDATION', 'Please give a reason for rejecting.');
  }
  ...
  if (!canActOnLeave(actor, row) && actor.role !== 'super_admin' && actor.role !== 'admin') {
    return fail(403, 'FORBIDDEN', 'You do not have permission to reject this leave.');
  }

  const now = Date.now();
  db.prepare(
    'UPDATE leave_requests SET status = ?, reviewed_by = ?, reviewed_at = ?, reviewed_note = ? WHERE id = ?'
  ).run('rejected', user.id, now, reason, leaveId);

  const lt = db.prepare('SELECT name FROM leave_types WHERE id = ?').get(row.leave_type_id) as any;
  notify(
    row.user_id,
    'Leave rejected',
    `Your ${lt?.name || 'Leave'} request (${row.start_date} to ${row.end_date}) was rejected — "${reason}"`,
    { type: 'error', link: '/leaves' }
  );
